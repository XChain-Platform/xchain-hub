...
function env(pubkey, sender) {
  let e = {
    sender: sender || 'addr-whatever'
  };
  if (pubkey !== undefined) e.sig_pubkey = pubkey;
  return e;
}
function registerSuite1Suite1Part1(cls, method) {
...
function registerSuite1Suite1Part2(cls, method) {
  it('fails closed on a non-string sig_pubkey', function () {
    const pm = {
      validatorPubkeys: new Map([['addr-other', KEY_REGISTRY]]),
      effectiveSignerSet: new Set([KEY_CHAIN])
    };
    expect(call(cls, method, pm, env({
      toString: () => KEY_CHAIN
    }))).to.equal(false);
  });
...
function registerVoteAdmissionFollowsTheChainEffectiveSigneSuite2Part1() {
  for (const {
    name,
    cls,
    method
    ...
    describe(name + '.' + method, function () {
      registerSuite1Suite1Part1.call(this, cls, method);
      registerSuite1Suite1Part2.call(this, cls, method);
    });
  }
  for (const {
    name,
...
describe('one key is one vote (count-mode forgery bound)', function () {
  ...
  it('OracleConsensus tallies distinct member keys against the locked snapshot', function () {
    ...
    const pending = {
      memberPubkeys: new Set([KEY_CHAIN, KEY_REGISTRY])
    };
  ...
  it('CrossChainEngine tallies distinct member keys against the locked snapshot', function () {
    ...
    const pending = {
      memberPubkeys: new Set([KEY_CHAIN])
    };
