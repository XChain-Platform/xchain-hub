...
// The indexer renamed its copy to snake_case; the camelCase name is what an indexer
// origin from before that rename carries, so the first of the two that exists is read.
const MIRROR = ['provider_registry.js', 'providerRegistry.js']
    .map((name) => path.join(path.dirname(REPO_ROOT), 'xchain-indexer', 'src', 'attestation', name))
    .find((p) => fs.existsSync(p)) ||
    path.join(path.dirname(REPO_ROOT), 'xchain-indexer', 'src', 'attestation', 'provider_registry.js');
...
// (xchain-indexer/src/attestation/provider_registry.js PROVIDERS), because it
