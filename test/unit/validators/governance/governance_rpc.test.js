...
function capabilityRegistryListStateSuite1MakeReg() {
  let db = capabilityRegistryListStateSuite1MakeDb();
  let reg = new CapabilityRegistry({
    db,
    p2pConfig: {}
  });
  return {
...
function registerCapabilityRegistryListStateSuite1Part1() {
  it('no filters: selects the explorer column set, default LIMIT 200', async function () {
    let {
      reg,
      conn
    } = capabilityRegistryListStateSuite1MakeReg();
    await reg.listState();
    let [query, args] = conn.query.firstCall.args;
    for (let col of ['id', 'signing_pubkey', 'capability', 'qualified', 'self_test_ok', 'enabled', 'qualified_at_block', 'updated_at']) expect(query).to.contain(col);
...
function registerGovernanceCapabilityReadRPCQueriesSuite2Part1() {
  afterEach(function () {
    sinon.restore();
  });
...
function registerGovernanceCapabilityReadRPCQueriesSuite2Part2() {
...
function registerGovernanceCapabilityReadRPCQueriesSuite2Part3() {
  ...
  describe('CapabilityRegistry.listState()', function () {
    registerCapabilityRegistryListStateSuite1Part1.call(this);
  });
}
describe('governance/capability read RPC queries', function () {
  registerGovernanceCapabilityReadRPCQueriesSuite2Part1.call(this);
  registerGovernanceCapabilityReadRPCQueriesSuite2Part2.call(this);
  registerGovernanceCapabilityReadRPCQueriesSuite2Part3.call(this);
});
