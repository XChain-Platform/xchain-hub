...
// Locate the project root by walking up to the lockfile rather than relying
// on a fixed number of '..' hops from the test directory.
...
//
// These HIGH advisories require dependency upgrades because lockfile
// re-resolution alone cannot select a safe version.
//
...
// Which of the advisories above are reachable here at all. An advisory for a
// package this tree never pulls in is not applicable rather than silently
// passing.
...
function registerAdvisoryTests(pkg, lock) {
    advisories.forEach(function (adv) {
        ...
        it(`ADV-2: every ${adv.name} entry in package-lock.json is at or above ${floor}`, function () {
            if (!present) return this.skip();
            const entries = lockEntries(lock, adv.name);
...
function registerRuntimeTests(root, lock) {
    ...
    // file collector. Exercise that boundary so the breakage surfaces here.
    ...
    it('ADV-5: the installed axios reports a patched runtime version', function () {
        if (!lockEntries(lock, 'axios').length) return this.skip();
    ...
    it('ADV-10: the installed mariadb reports a patched runtime version', function () {
        if (!lockEntries(lock, 'mariadb').length) return this.skip();
...
describe('Security: remediated dependency advisories @regression @tier4', function () {
    ...
    registerAdvisoryTests(pkg, lock);
    registerRuntimeTests(root, lock);
});
