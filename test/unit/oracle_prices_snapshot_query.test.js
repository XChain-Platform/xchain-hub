...
const {
  buildOraclePricesSnapshotQuery,
  MAX_SNAPSHOT_ROWS
...
function registerLatestModeDashboardCurrentPerFeedSuite1Part1() {
  it('selects the MAX(effective_at) row per (source_address,coin,tick,fiat), not per id', function () {
    const built = buildOraclePricesSnapshotQuery({
      latest: true,
      now: 1000
    });
    const {
  ...
  it('gates the MAX(effective_at) subquery to now, hiding future-dated rows', function () {
    const sql = sqlOf(buildOraclePricesSnapshotQuery({
      latest: true,
      now: 1000
    }));
...
function registerLatestModeDashboardCurrentPerFeedSuite1Part2() {
...
function registerBuildOraclePricesSnapshotQuerySuite2Part1() {
  describe('default (page) mode - indexer bootstrap, must not change', function () {
    ...
    it('clamps a negative / non-numeric since_id to 0 (no NaN bind)', function () {
      expect(buildOraclePricesSnapshotQuery({
        since: -7
      }).params[0]).to.equal(0);
      expect(buildOraclePricesSnapshotQuery({
        since: 'abc'
    ...
    it('clamps limit into [1, cap]', function () {
      expect(buildOraclePricesSnapshotQuery({
        limit: 999999
      }).params[1]).to.equal(MAX_SNAPSHOT_ROWS);
      expect(buildOraclePricesSnapshotQuery({
        limit: 0
      }).params[1]).to.equal(MAX_SNAPSHOT_ROWS);
      expect(buildOraclePricesSnapshotQuery({
      ...
      expect(buildOraclePricesSnapshotQuery({
        limit: 250
      }).params[1]).to.equal(250);
    });
  });
  describe('latest mode - dashboard current-per-feed', function () {
    registerLatestModeDashboardCurrentPerFeedSuite1Part1.call(this);
    registerLatestModeDashboardCurrentPerFeedSuite1Part2.call(this);
  });
}
describe('buildOraclePricesSnapshotQuery', function () {
  registerBuildOraclePricesSnapshotQuerySuite2Part1.call(this);
});
