...
function registerValidateActivationSuite1Part1() {
  it('accepts a coherent activation', function () {
    expect(activation.validateActivation(activatedCfg())).to.deep.equal([]);
    expect(activation.describeActivation(activatedCfg())).to.equal('ACTIVE (REWARD_SHARE=0.25, 2 genesis verifiers)');
  });
  it('rejects activation with no genesis verifiers (the tranche is unearnable)', function () {
    let problems = activation.validateActivation(activatedCfg({
      GENESIS_VERIFIERS: []
    }));
  ...
  it('rejects a negative or non-numeric REWARD_SHARE', function () {
    expect(activation.validateActivation(activatedCfg({
      REWARD_SHARE: '-0.1'
    })).join(' ')).to.include('outside [0,1]');
    expect(activation.validateActivation(activatedCfg({
      REWARD_SHARE: 'a quarter'
    })).join(' ')).to.include('not a number');
  });
  ...
  it('rejects duplicate genesis verifiers (case-insensitive)', function () {
    let problems = activation.validateActivation(activatedCfg({
      GENESIS_VERIFIERS: [PK1, PK1.toUpperCase()]
    }));
    expect(problems.join(' ')).to.include('duplicate');
  });
}
function registerValidateActivationSuite1Part2() {
  ...
  it('rejects an out-of-range MIN_PASS_RATE_BPS', function () {
    expect(activation.validateActivation(activatedCfg({
      MIN_PASS_RATE_BPS: 20000
    })).join(' ')).to.include('MIN_PASS_RATE_BPS 20000');
  });
  it('rejects a non-integer block count', function () {
    expect(activation.validateActivation(activatedCfg({
      CONFIRM_DEPTH: '100.5'
    })).join(' ')).to.include('CONFIRM_DEPTH is not an integer');
  });
  it('does not apply the deadlock checks while the tier is inert', function () {
    ...
    let inert = Object.assign({}, CANONICAL, {
      PROOF_WINDOW_BLOCKS: 1,
      REWARD_PASS_WINDOW_BLOCKS: 1
    });
    expect(activation.validateActivation(inert)).to.deep.equal([]);
  });
}
function registerFullnodeActivationSuite2Part1() {
  ...
  describe('validateActivation', function () {
    registerValidateActivationSuite1Part1.call(this);
    registerValidateActivationSuite1Part2.call(this);
  });
}
function registerFullnodeActivationSuite2Part2() {
  describe('diffCanonical', function () {
    it('passes an override that matches the bundle', function () {
      expect(activation.diffCanonical({
        REWARD_SHARE: '0'
      }, CANONICAL)).to.deep.equal([]);
    });
    ...
    it('reports a divergent consensus knob', function () {
      let problems = activation.diffCanonical({
        CHALLENGE_INTERVAL_BLOCKS: 10
      }, CANONICAL);
      expect(problems.length).to.equal(1);
    ...
    it('reports a verifier list the bundle does not carry', function () {
      expect(activation.diffCanonical({
        GENESIS_VERIFIERS: [PK1]
      }, CANONICAL).join(' ')).to.include('GENESIS_VERIFIERS');
    });
    ...
    it('covers every consensus key the bundle declares', function () {
      ...
      expect(keys).to.include.members(['REWARD_SHARE', 'GENESIS_VERIFIERS', 'CHALLENGE_INTERVAL_BLOCKS', 'CONFIRM_DEPTH', 'PROOF_WINDOW_BLOCKS', 'VERDICT_ACCEPT_WINDOW_BLOCKS', 'REWARD_PASS_WINDOW_BLOCKS', 'MIN_PASS_RATE_BPS']);
      expect(keys.some(k => k.charAt(0) === '$')).to.equal(false);
    });
  });
}
function registerFullnodeActivationSuite2Part3() {
  describe('mergeWithCanonical', function () {
    it('keeps the bundle values and adds the operator local keys', function () {
      let merged = activation.mergeWithCanonical(CANONICAL, {
        BTC_RPC: 'http://coin'
      });
    ...
    it('strips $-prefixed descriptors', function () {
      let merged = activation.mergeWithCanonical({
        A: 1,
        $regtestSidecar: 'x.json'
      }, null);
...
describe('fullnode_activation', function () {
  registerFullnodeActivationSuite2Part1.call(this);
  registerFullnodeActivationSuite2Part2.call(this);
  registerFullnodeActivationSuite2Part3.call(this);
});
let xChainHubAssertCanonicalFullnodeSuite3WarnStub;
function registerXChainHubAssertCanonicalFullnodeSuite3Part1() {
  beforeEach(function () {
    xChainHubAssertCanonicalFullnodeSuite3WarnStub = sinon.stub(console, 'warn');
  });
  afterEach(function () {
    xChainHubAssertCanonicalFullnodeSuite3WarnStub.restore();
    delete process.env.XCHAIN_HUB_SKIP_FULLNODE_ASSERT;
  });
  it('accepts a local-keys-only override on mainnet', function () {
    expect(() => makeHub('mainnet').assertCanonicalFullnode({
      BTC_RPC: 'http://coin'
    })).to.not.throw();
  });
  it('refuses a divergent challenge interval on mainnet', function () {
    let err = null;
    try {
      makeHub('mainnet').assertCanonicalFullnode({
        CHALLENGE_INTERVAL_BLOCKS: 10
      });
    } catch (e) {
      err = e;
  ...
  it('refuses on testnet too', function () {
    expect(() => makeHub('testnet').assertCanonicalFullnode({
      REWARD_SHARE: '0.5'
  ...
  it('warns instead of throwing on regtest (venues run their own cadence)', function () {
    expect(() => makeHub('regtest').assertCanonicalFullnode({
      CHALLENGE_INTERVAL_BLOCKS: 4
    })).to.not.throw();
    expect(xChainHubAssertCanonicalFullnodeSuite3WarnStub.calledWithMatch(/FULLNODE config problem/)).to.equal(true);
  });
}
function registerXChainHubAssertCanonicalFullnodeSuite3Part2() {
  it('warns instead of throwing in standalone mode (no HUB_NETWORK)', function () {
    expect(() => makeHub(undefined).assertCanonicalFullnode({
      CHALLENGE_INTERVAL_BLOCKS: 4
    })).to.not.throw();
    expect(xChainHubAssertCanonicalFullnodeSuite3WarnStub.calledWithMatch(/FULLNODE config problem/)).to.equal(true);
  });
  it('XCHAIN_HUB_SKIP_FULLNODE_ASSERT=1 bypasses loudly, even on mainnet', function () {
    ...
    expect(xChainHubAssertCanonicalFullnodeSuite3WarnStub.calledWithMatch(/XCHAIN_HUB_SKIP_FULLNODE_ASSERT/)).to.equal(true);
  });
...
describe('XChainHub.assertCanonicalFullnode', function () {
  registerXChainHubAssertCanonicalFullnodeSuite3Part1.call(this);
  registerXChainHubAssertCanonicalFullnodeSuite3Part2.call(this);
});
