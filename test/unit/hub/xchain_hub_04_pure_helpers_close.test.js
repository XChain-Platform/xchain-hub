...
function registerFeature17pureHelpersClosePart1() {
  ...
  it('resolveBtcIndexerUrl prefers BTC_INDEXER_API_URL, falls back to BTC_INDEXER_URL, then configs', async function () {
    ...
    expect(await feature17pureHelpersCloseHub.resolveBtcIndexerUrl()).to.equal('http://cfg-host:4001');
    ...
    expect(await feature17pureHelpersCloseHub.resolveBtcIndexerUrl()).to.equal('http://alias:4000');
    process.env.BTC_INDEXER_API_URL = 'http://canonical:4000';
    expect(await feature17pureHelpersCloseHub.resolveBtcIndexerUrl()).to.equal('http://canonical:4000');
    delete process.env.BTC_INDEXER_API_URL;
    delete process.env.BTC_INDEXER_URL;
  });
