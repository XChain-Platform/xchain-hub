...
function signOf(eng, idx) {
            return IDS[idx].sign(eng.canonical(EPOCH, LEDGER_HASH));
        }
...
it('judges an early signature by the same rule as a live one', async function () {
            ...
            const eng = makeEngine({});
            eng.handleMessage({ type: 'XROLLCALL_SIGN',
                                 data: { epoch: EPOCH, pubkey: PKS[1], sig: IDS[1].sign(eng.canonical(60, LEDGER_HASH)) } });
            await eng.tick();
...
it('drops a signature that does not verify over OUR canonical', async function () {
            ...
            const wrongEpochSig = IDS[1].sign(eng.canonical(60, LEDGER_HASH));
            eng.handleMessage({ type: 'XROLLCALL_SIGN',
...
it('dedupes by pubkey: the first verified signature wins', async function () {
            ...
            eng.handleMessage({ type: 'XROLLCALL_SIGN', data: { epoch: EPOCH, pubkey: PKS[1], sig: real } });
            // A second, differently-shaped payload for the same key must not replace it.
            eng.handleMessage({ type: 'XROLLCALL_SIGN', data: { epoch: EPOCH, pubkey: PKS[1], sig: 'b'.repeat(128) } });
...
it('ignores malformed pubkeys, malformed signatures and unknown epochs', async function () {
            ...
            eng.handleMessage({ type: 'XROLLCALL_SIGN', data: { epoch: EPOCH, pubkey: 'zz', sig: signOf(eng, 1) } });
            eng.handleMessage({ type: 'XROLLCALL_SIGN', data: { epoch: EPOCH, pubkey: PKS[1], sig: 'short' } });
            eng.handleMessage({ type: 'XROLLCALL_SIGN', data: { epoch: 999, pubkey: PKS[1], sig: signOf(eng, 1) } });
            eng.handleMessage({ type: 'SOMETHING_ELSE', data: { epoch: EPOCH } });
...
it('applies NO stake floor and computes NO quorum', async function () {
            ...
            await eng.tick();
            eng.handleMessage({ type: 'XROLLCALL_SIGN',
                                 data: { epoch: EPOCH, pubkey: PKS[1], sig: IDS[1].sign(eng.canonical(EPOCH, LEDGER_HASH)) } });
            assert.strictEqual(eng.rounds.get(EPOCH).sigs.has(PKS[1]), true);
        });
});
});
