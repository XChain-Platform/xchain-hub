    ...
    Bitcoin: {
      regtest: {
        ...
        'xchain-indexer': {
          ...
          db_host: 'mariadb',
          db_port: '3306',
          name: 'XCHAIN_BTC_REGTEST',
          user: 'indexer_user',
          pass: 'INDEXER-DB-SECRET'
        }
      }
    }
  };
...
async function bootController(env) {
  let controller = null;
  const mockApp = {use:sinon.stub(),get:sinon.stub(),post:sinon.stub(),set:sinon.stub(),listen:sinon.stub().callsFake((port,host,cb)=>{if(cb)cb();})};
...
let getallconfigsCredentialRedactionHandlerSuite1Controller, getallconfigsCredentialRedactionHandlerSuite1HubStubs;
function registerGetallconfigsCredentialRedactionHandlerSuite1Part1() {
  before(async function () {
    const booted = await bootController({HUB_API_KEY:'test-hub-key'});
    getallconfigsCredentialRedactionHandlerSuite1Controller = booted.controller;
    getallconfigsCredentialRedactionHandlerSuite1HubStubs = booted.hubStubs;
  });
  afterEach(function () {
    sinon.resetHistory();
  });
  ...
  it('redacts every password when the caller does not ask for secrets', async function () {
    const out = await getallconfigsCredentialRedactionHandlerSuite1Controller.getallconfigs({});
  ...
  it('serves the real credentials when include_secrets is set', async function () {
    const out = await getallconfigsCredentialRedactionHandlerSuite1Controller.getallconfigs({include_secrets:true});
...
function registerGetallconfigsCredentialRedactionHandlerSuite1Part2() {
  ...
  it('redacts on a near-miss value rather than guessing the caller meant yes', async function () {
    for (const v of ['yes', 'false', 0, null]) {
      const out = await getallconfigsCredentialRedactionHandlerSuite1Controller.getallconfigs({include_secrets:v});
  ...
  it('passes since_updated_at through untouched (delta polls keep working)', async function () {
    await getallconfigsCredentialRedactionHandlerSuite1Controller.getallconfigs({since_updated_at:1788121400});
    expect(getallconfigsCredentialRedactionHandlerSuite1HubStubs.getAllConfigs.calledWith(1788121400)).to.equal(true);
  });
  it('redacts the delta response too, not just the full tree', async function () {
    const out = await getallconfigsCredentialRedactionHandlerSuite1Controller.getallconfigs({since_updated_at:1788121400});
    expect(out.configs.Bitcoin.regtest.bitcoind.pass).to.equal(redaction.REDACTED);
  });
  it('leaves the hub\'s own copy of the tree unredacted (internal readers share it)', async function () {
    const served = configTree();
    getallconfigsCredentialRedactionHandlerSuite1HubStubs.getAllConfigs.resolves(served);
    await getallconfigsCredentialRedactionHandlerSuite1Controller.getallconfigs({});
    expect(served.Bitcoin.regtest.bitcoind.pass).to.equal('NODE-RPC-SECRET');
    getallconfigsCredentialRedactionHandlerSuite1HubStubs.getAllConfigs.resolves(configTree());
  });
  it('still answers a DB failure with the error envelope, carrying no config', async function () {
    getallconfigsCredentialRedactionHandlerSuite1HubStubs.getAllConfigs.rejects(new Error('db down'));
    const out = await getallconfigsCredentialRedactionHandlerSuite1Controller.getallconfigs({include_secrets:true});
...
describe('getallconfigs credential redaction (handler)', function () {
  registerGetallconfigsCredentialRedactionHandlerSuite1Part1.call(this);
  registerGetallconfigsCredentialRedactionHandlerSuite1Part2.call(this);
});
