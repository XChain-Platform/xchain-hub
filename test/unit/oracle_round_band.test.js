...
function registerOracleRoundBandSuite1Part1() {
  ...
  describe('roundBand', function () {
    it('bounds the future side at current + tolerance', function () {
      let b = band.roundBand({
        nowMs: 1000 + 100 * 60000,
        epochStartMs: 1000,
...
function registerOracleRoundBandSuite1Part2() {
  describe('isRoundImplausible', function () {
    let b;
    beforeEach(function () {
      b = band.roundBand({
        nowMs: 1000 + 51688 * 60000,
        epochStartMs: 1000,
        roundIntervalMs: 60000
      });
    });
...
describe('oracle_round_band', function () {
  registerOracleRoundBandSuite1Part1.call(this);
  registerOracleRoundBandSuite1Part2.call(this);
});
...
function priceAggregatorOutOfBandRoundRejectionSuite2PushFor(round) {
  return {
    round: round,
...
function registerPriceAggregatorOutOfBandRoundRejectionSuite2Part1() {
  afterEach(function () {
    sinon.restore();
  });
  it('refuses the sentinel round BEFORE any signature or DB work', async function () {
    ...
    let res = await agg.receiveValidatedRound('BTC', priceAggregatorOutOfBandRoundRejectionSuite2PushFor(SENTINEL_ROUND));
    expect(res).to.deep.equal({
      accepted: false,
      reason: 'implausible round'
    });
  ...
  it('lets an IN-BAND round through the band check to the normal pipeline', async function () {
    let hub = hubOnRound(51688);
    let agg = new PriceAggregator(hub);
    let res = await agg.receiveValidatedRound('BTC', priceAggregatorOutOfBandRoundRejectionSuite2PushFor(51688));
  ...
  it('does NOT bound the past: a round from days ago still gets through', async function () {
    ...
    let hub = hubOnRound(51688);
    let agg = new PriceAggregator(hub);
    let res = await agg.receiveValidatedRound('BTC', priceAggregatorOutOfBandRoundRejectionSuite2PushFor(1));
    expect(res.reason).to.not.equal('implausible round');
  });
...
function registerPriceAggregatorOutOfBandRoundRejectionSuite2Part2() {
...
describe('PriceAggregator out-of-band round rejection', function () {
  registerPriceAggregatorOutOfBandRoundRejectionSuite2Part1.call(this);
  registerPriceAggregatorOutOfBandRoundRejectionSuite2Part2.call(this);
});
let oracleRoundDiagnosticsBandReportSuite3Hub, oracleRoundDiagnosticsBandReportSuite3Or;
// Route the diagnostics reads by SQL shape; only the band query returns rows.
function oracleRoundDiagnosticsBandReportSuite3StubReads(outOfBandRows, opts) {
  oracleRoundDiagnosticsBandReportSuite3Hub.db.doQuery = sinon.stub().callsFake(async sql => {
...
function registerOracleRoundDiagnosticsBandReportSuite3Part1() {
  beforeEach(function () {
    ...
    oracleRoundDiagnosticsBandReportSuite3Hub = createMockHub({
      p2pConfig: {
    ...
    oracleRoundDiagnosticsBandReportSuite3Or = new OracleRound(oracleRoundDiagnosticsBandReportSuite3Hub);
  });
  afterEach(function () {
    sinon.restore();
  });
  it('names a stored round past the band, so a gap scan can drop it', async function () {
    oracleRoundDiagnosticsBandReportSuite3StubReads([{
      round_number: SENTINEL_ROUND
    }]);
    let info = await oracleRoundDiagnosticsBandReportSuite3Or.getSubmissionsInfo();
  ...
  it('warns ONCE for a standing sentinel, not on every diagnostics poll', async function () {
    ...
    await oracleRoundDiagnosticsBandReportSuite3Or.getSubmissionsInfo();
    await oracleRoundDiagnosticsBandReportSuite3Or.getSubmissionsInfo();
    warn.restore();
    expect(warns.filter(m => /past the plausible band/.test(m))).to.have.length(1);
  });
}
function registerOracleRoundDiagnosticsBandReportSuite3Part2() {
  ...
  it('queries above the band MAX, not above the current round', async function () {
    oracleRoundDiagnosticsBandReportSuite3StubReads([]);
    await oracleRoundDiagnosticsBandReportSuite3Or.getSubmissionsInfo();
    let call = oracleRoundDiagnosticsBandReportSuite3Hub.db.doQuery.getCalls().find(c => /round_number > \?/.test(c.args[0]));
  ...
  it('surfaces the aggregator write-time refusal count', async function () {
    oracleRoundDiagnosticsBandReportSuite3StubReads([]);
    oracleRoundDiagnosticsBandReportSuite3Hub.priceAggregator = {
      implausibleRoundRejections: 3
    };
    let info = await oracleRoundDiagnosticsBandReportSuite3Or.getSubmissionsInfo();
    expect(info.implausibleRoundRejections).to.equal(3);
  });
}
function registerOracleRoundDiagnosticsBandReportSuite3Part3() {
  it('skips the read entirely when the schedule is unresolvable, and says so with a null band', async function () {
    oracleRoundDiagnosticsBandReportSuite3StubReads([{
      round_number: SENTINEL_ROUND
    }]);
    oracleRoundDiagnosticsBandReportSuite3Or.roundInterval = 0; // unresolvable schedule
    let info = await oracleRoundDiagnosticsBandReportSuite3Or.getSubmissionsInfo();
    expect(info.roundBand).to.equal(null);
    expect(info.implausibleRounds).to.deep.equal([]);
    expect(oracleRoundDiagnosticsBandReportSuite3Hub.db.doQuery.getCalls().some(c => /round_number > \?/.test(c.args[0]))).to.equal(false);
  });
}
describe('OracleRound diagnostics band report', function () {
  registerOracleRoundDiagnosticsBandReportSuite3Part1.call(this);
  registerOracleRoundDiagnosticsBandReportSuite3Part2.call(this);
  registerOracleRoundDiagnosticsBandReportSuite3Part3.call(this);
});
