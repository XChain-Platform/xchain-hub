...
describe('OraclePublisher PRICE batch rail', function () {
    ...
    // Measured on public testnet 2026-09-07 after the v0.15.0 roll: every
    // validator hub booted onto a buffer holding its whole finalized history (~1400
    // rounds, 704 closed windows) because the observation prune keys on rows a
    // validator never gets, and the hourly sweep then re-published windows the chain
    // already carried, four an hour, at a DOGE fee apiece. Two seams close it: the
    // aggregator hands every landed batch to noteBatchLanded (every hub hears every
    // push), and the sweep asks the landing chain's indexer about the backlog first.
    describe('shedding windows the chain already carries', function () {
        ...
        describe('noteBatchLanded', function () {
            it('sheds the covered rounds from memory AND the buffer file and memoizes the window', async function () {
                let h = makePublisher();
                await h.p.start();
            ...
            it('keeps a PARTLY covered window re-proposable over its remaining rounds only', async function () {
                let h = makePublisher();
                await h.p.start();
            ...
            it('disarms a takeover and a grace timer for a window that has fully landed', async function () {
                let h = makePublisher();
                await h.p.start();
        ...
        describe('reconciling the backlog against the landing chain before a sweep', function () {
            it('asks the DOGE indexer for the whole pending span, with the federation key, and sheds what it names', async function () {
            ...
            it('the sweep then re-proposes ONLY the unlanded windows, so nothing already on chain is paid for twice', async function () {
                let h = makePublisher({ signerOpts: { met: false } });
                await h.p.start();
                for (let r = 0; r < 25; r++) h.p._buffer.set(r, bufferedFixture(r));
            ...
            it('a batch the chain carries under a DIFFERENT window split still sheds every buffered round it covers', async function () {
                // The fleet re-windowed from 6 rounds to 2, so on-chain batches no longer
            ...
            it('fails OPEN when the indexer is unreachable or too old to answer, and logs the reason once', async function () {
                let h = makePublisher();
                await h.p.start();
                for (let r = 0; r < 13; r++) h.p._buffer.set(r, bufferedFixture(r));
            ...
            it('does not ask at all when nothing is pending', async function () {
                let h = makePublisher();
                await h.p.start();
        ...
        describe('a retracted batch gets its rounds back', function () {
            ...
            it('restores the shed window from price_snapshots when its markers are cleared, so it can be re-published', async function () {
            ...
            it('restores only what the buffer lacks and only from v0-proofed finalized rows', async function () {
