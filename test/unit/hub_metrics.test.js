...
function registerHubOracleRoundHeartbeatMetricsItemA98d6746Suite1Part1() {
  ...
  it('renders freshness, round number and skip streak from live oracle state', function () {
    ...
    expect(installHubOracleMetrics(observability, {
      getOracle: () => oracle
    })).to.equal(true);
  ...
  it('keeps a wedged round loop visible as a frozen series', function () {
    ...
    expect(observability.registry.render()).to.match(/xchain_oracle_last_finalized_round_timestamp_seconds 1754870400\b/);
    oracle.lastSuccessfulRoundTime = 1754870700000;
    expect(observability.registry.render()).to.match(/xchain_oracle_last_finalized_round_timestamp_seconds 1754870700\b/);
  });
}
function registerHubOracleRoundHeartbeatMetricsItemA98d6746Suite1Part2() {
  it('leaves the freshness series absent until the first finalization', function () {
    const observability = realObservability();
    installHubOracleMetrics(observability, {
      getOracle: () => ({
        lastSuccessfulRoundTime: null,
        currentRound: 0,
        consecutiveSkippedRounds: 0
      })
    });
    ...
    // still starting up.
    expect(observability.registry.render()).to.not.match(/xchain_oracle_last_finalized_round_timestamp_seconds \d/);
  });
...
function registerHubOracleRoundHeartbeatMetricsItemA98d6746Suite1Part3() {
...
function registerHubOracleRoundHeartbeatMetricsItemA98d6746Suite1Part4() {
  it('never walks the single-source counter backwards when the oracle is re-minted', function () {
    const observability = realObservability();
    let consensus = {
      _roundTimeouts: 0,
      _singleSourceRounds: 9
    };
    installHubOracleMetrics(observability, {
      getOracle: () => ({
        oracleConsensus: consensus
...
function registerHubOracleRoundHeartbeatMetricsItemA98d6746Suite1Part5() {
  it('refuses to register without a registry at all', function () {
    expect(installHubOracleMetrics({
      registry: null
    }, {
      getOracle: () => ({})
    })).to.equal(false);
...
describe('hub oracle-round heartbeat metrics (item a98d6746)', function () {
  registerHubOracleRoundHeartbeatMetricsItemA98d6746Suite1Part1.call(this);
  registerHubOracleRoundHeartbeatMetricsItemA98d6746Suite1Part2.call(this);
