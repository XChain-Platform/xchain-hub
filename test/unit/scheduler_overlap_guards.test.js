...

{
    ...
    async function aPassFiringWhileTheBooksTest2() {
    ...
    function crosschaindexengineDiscoverandmatchOverlapGuardSuite1() {
        afterEach(function () { sinon.restore(); });
        it('a pass firing while the books are still loading proposes nothing', aPassFiringWhileTheBooksTest2);
...

{
    ...
    async function aSweepFiringWhileThePendingTest5() {
    ...
    async function aRejectedPendingSetFetchDoesTest6() {
        const pub   = makePublisher();
        const bcast = sinon.stub().resolves({ txid: 'replay-txid' });
        pub.setBroadcastHook(bcast);
        writeQueue([leaderEntry()]);
    ...
    function attestationpublisherProcessqueueOverlapGuardSuite4() {
        beforeEach(function () {
...

{
    ...
    async function theNextRoundTickIsSkippedTest8() {
    ...
    async function aRejectedPriceFetchDoesNotTest9() {
        clock = sinon.useFakeTimers({ now: or.epochStart + 1000, toFake: ['Date'] });
    ...
    function oracleroundExecuteroundOverlapGuardSuite7() {
        ...
        it('the next round tick is skipped while the previous round is still fetching prices', theNextRoundTickIsSkippedTest8);
