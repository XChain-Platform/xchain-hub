...
describe('Boundary: Config Management', registerBoundaryConfigManagement);

function registerBoundaryConfigManagement() {
    beforeEach(function () {
        mockDb = {
    ...
    describe('applyConfig()', registerApplyConfig);
    // addParametersFromJson: boundary tests
    describe('addParametersFromJson()', registerAddParametersFromJson);
}

function registerApplyConfig() {
    it('skips a coin key that is an empty string', testSkipsACoinKeyThatIsAnEmptyString);
    it('batches one row per known parameter present in the module', testBatchesOneRowPerKnownParameterPresentInTheModule);
    it('silently ignores unknown parameter names not in the allowlist', testSilentlyIgnoresUnknownParameterNamesNotInTheAllowlist);
    it('skips parameters whose value is null', testSkipsParametersWhoseValueIsNull);
    it('skips parameters whose value is undefined', testSkipsParametersWhoseValueIsUndefined);
    it('does NOT skip a parameter whose value is an empty string', testDoesNOTSkipAParameterWhoseValueIsAnEmptyString);
    it('makes no DB write for an empty config object', testMakesNoDBWriteForAnEmptyConfigObject);
    it('handles multiple coins, networks, and modules and produces the correct total row count', testHandlesMultipleCoinsNetworksAndModulesAndProducesTheCorrectTotalRow);
    it('only writes params that are present when a module has a partial parameter set', testOnlyWritesParamsThatArePresentWhenAModuleHasAPartial);
}
async function testSkipsACoinKeyThatIsAnEmptyString() {
    const json = {
...
async function testBatchesOneRowPerKnownParameterPresentInTheModule() {
    const json = {
        BTC: {
            mainnet: {
                decoder: {
...
async function testSilentlyIgnoresUnknownParameterNamesNotInTheAllowlist() {
    const json = {
        BTC: {
            mainnet: {
                decoder: {
...
async function testSkipsParametersWhoseValueIsNull() {
    const json = {
        BTC: {
            mainnet: {
...
async function testSkipsParametersWhoseValueIsUndefined() {
    const json = {
        BTC: {
            mainnet: {
...
async function testDoesNOTSkipAParameterWhoseValueIsAnEmptyString() {
    const json = {
        BTC: {
            mainnet: {
                decoder: { host: '' }
...
async function testMakesNoDBWriteForAnEmptyConfigObject() {
    await hub.applyConfig({});
    expect(mockDb.setParams.called).to.be.false;
}
async function testHandlesMultipleCoinsNetworksAndModulesAndProducesTheCorrectTotalRow() {
...
async function testOnlyWritesParamsThatArePresentWhenAModuleHasAPartial() {
    const json = {
        DOGE: {
            mainnet: {
...
function registerAddParametersFromJson() {
    it('calls applyConfig directly when no consensus instance is active', testCallsApplyConfigDirectlyWhenNoConsensusInstanceIsActive);
    it('delegates to consensus.propose and skips applyConfig when consensus is active', testDelegatesToConsensusProposeAndSkipsApplyConfigWhenConsensusIsActive);
    it('returns true for an empty config object', testReturnsTrueForAnEmptyConfigObject);
}
async function testCallsApplyConfigDirectlyWhenNoConsensusInstanceIsActive() {
    hub.consensus = null;
...
async function testDelegatesToConsensusProposeAndSkipsApplyConfigWhenConsensusIsActive() {
...
async function testReturnsTrueForAnEmptyConfigObject() {
    hub.consensus = null;
