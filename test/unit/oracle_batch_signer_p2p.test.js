...
function baseRounds() {
  let out = [];
  for (let i = 0; i < 6; i++) {
    out.push({round:300+i,timestamp:1700200000+i*600,btcBlockHeight:6000+i,pairs:[{pair:'BTC/USD',price:String(61000+i)},{pair:'DOGE/USD',price:String(0.1+i)}]});
  }
  return out;
}
function snapshotRows(rounds) {
  let rows = [];
  for (let r of rounds) for (let p of r.pairs) rows.push({round_number:r.round,coin_pair:p.pair,price:String(p.price),reference_block:r.btcBlockHeight,block_timestamp:r.timestamp,status:'finalized'});
  return rows;
}
function memDb(rows) {
  return {
    ...
    async doQuery(sql, params) {
      ...
      return rows.filter(r => r.round_number >= first && r.round_number <= last && r.status === status).map(r => Object.assign({}, r));
    }
  };
}
...
let oracleBatchSignerOverRealP2PTransportSuite1Managers = [];
let oracleBatchSignerOverRealP2PTransportSuite1Signers = [];
async function oracleBatchSignerOverRealP2PTransportSuite1BootMesh(n, perNodeRounds) {
  let ports = [];
  ...
  for (let i = 0; i < n; i++) {
    ...
    await pm.start();
    oracleBatchSignerOverRealP2PTransportSuite1Managers.push(pm);
    let hub = {
      db: memDb(snapshotRows(perNodeRounds ? perNodeRounds(i) : baseRounds())),
      network: 'regtest',
      p2pConfig: {
        ORACLE_BATCH_SIGN_TIMEOUT_MS: '1500'
      },
      capabilitySnapshot: {
    ...
    signer.start();
    oracleBatchSignerOverRealP2PTransportSuite1Signers.push(signer);
  }

  // Every node must see n-1 open peers before a broadcast can reach quorum.
  await waitUntil(() => oracleBatchSignerOverRealP2PTransportSuite1Managers.every(pm => pm.getPeerStatus().filter(p => p.state === 'open').length >= n - 1), {timeoutMs:10000,label:'the '+n+'-node mesh to fully connect'});
  return {pubkeys,identities};
}
function registerOracleBatchSignerOverRealP2PTransportSuite1Part1() {
  this.timeout(20000);
  afterEach(async function () {
    for (let s of oracleBatchSignerOverRealP2PTransportSuite1Signers) s.stop();
    ...
    let stopping = oracleBatchSignerOverRealP2PTransportSuite1Managers.map(pm => pm.stop());
    for (let pm of oracleBatchSignerOverRealP2PTransportSuite1Managers) if (pm.httpServer && pm.httpServer.closeAllConnections) pm.httpServer.closeAllConnections();
    await Promise.all(stopping);
    oracleBatchSignerOverRealP2PTransportSuite1Managers = [];
    oracleBatchSignerOverRealP2PTransportSuite1Signers = [];
  });
  it('routes XPRICEB_SIGN_REQ/XPRICEB_SIGN through the real gossip layer to a quorum', async function () {
    let {pubkeys} = await oracleBatchSignerOverRealP2PTransportSuite1BootMesh(3);
  ...
  it('gives a dishonest leader no quorum across the real transport', async function () {
    await oracleBatchSignerOverRealP2PTransportSuite1BootMesh(3);
    let fabricated = baseRounds();
    fabricated[2].pairs[0].price = '1'; // a price no peer finalized

    let res = await oracleBatchSignerOverRealP2PTransportSuite1Signers[0].collectBatchSignatures(300, 305, 6005, fabricated);
    expect(res.met).to.equal(false);
    expect(res.sigs.length).to.equal(1);
    expect(oracleBatchSignerOverRealP2PTransportSuite1Signers[0].getStats().batchSignTimeouts).to.equal(1);
    for (let i = 1; i < oracleBatchSignerOverRealP2PTransportSuite1Signers.length; i++) {
      expect(oracleBatchSignerOverRealP2PTransportSuite1Signers[i].getStats().batchSignRefusals).to.equal(1);
      expect(oracleBatchSignerOverRealP2PTransportSuite1Signers[i].getStats().batchSignaturesProvided).to.equal(0);
    }
  });
}
describe('OracleBatchSigner over real P2P transport', function () {
  registerOracleBatchSignerOverRealP2PTransportSuite1Part1.call(this);
});
