...
async function bootApi({ envOverrides, hubOverrides, axiosPost } = {}) {
        ...
        const { mockExpress, mockServer } = makeApiSurfaceServer();
...
function registerResponsibleSetWideningTest() {
...
it('returns the same set AttestationRound.computeResponsibleSet computes for a seeded request', async function () {
    ...
    registerResponsibleSetWideningTest();
}

function registerResponsibleSetRefusalTests() {
...
function registerResponsibleSetSuite() {
describe('getattestationresponsibleset', function () {


        registerResponsibleSetRefusalTests();

        registerResponsibleSetResolutionTests();
    });
}

function registerAttestationConfigSuite() {
...
describe('attestation API surfaces (p2pConfig knobs + responsible-set read)', function () {
    ...
    registerAttestationConfigSuite();

    // ── Row 47: getattestationresponsibleset ─────────────────────────────────
    registerResponsibleSetSuite();
});
