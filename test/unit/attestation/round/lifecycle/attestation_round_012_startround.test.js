...
// ── _startRound ───────────────────────────────────────────────────────────
describe('AttestationRound', function () { beforeEach(hookAt3853); afterEach(hookAt3913); describe('_startRound()', function () { it('skips when provider is unknown', async function () {
            let hub = makeHub();
...
// ── _startRound ───────────────────────────────────────────────────────────
describe('AttestationRound', function () { beforeEach(hookAt3853); afterEach(hookAt3913); describe('_startRound()', function () { it('skips when provider module has no fetch()', async function () {
            let hub = makeHub();
...
// ── _startRound ───────────────────────────────────────────────────────────
describe('AttestationRound', function () { beforeEach(hookAt3853); afterEach(hookAt3913); describe('_startRound()', function () { it('skips when capability snapshot is empty', async function () {
...
// ── _startRound ───────────────────────────────────────────────────────────
describe('AttestationRound', function () { beforeEach(hookAt3853); afterEach(hookAt3913); describe('_startRound()', function () { it('skips (before the provider fetch) when responsible slots < redundancy (Pkg 7 / 87441a53)', async function () {
...
// ── _startRound ───────────────────────────────────────────────────────────
describe('AttestationRound', function () { beforeEach(hookAt3853); afterEach(hookAt3913); describe('_startRound()', function () { it('skips when this validator is not in the responsible set', async function () {
...
// ── _startRound ───────────────────────────────────────────────────────────
describe('AttestationRound', function () { beforeEach(hookAt3853); afterEach(hookAt3913); describe('_startRound()', function () { it('skips (before any fetch) when the request fee is below the provider min_fee', async function () {
            let myPubkey = 'aa'.repeat(32);
            let capSS = { getSnapshot: sinon.stub().resolves({ validators: [{ pubkey: myPubkey }] }) };
            let hub   = makeHub({ capabilitySnapshot: capSS });
            hub.getIdentity = () => makeIdentity(myPubkey);
...
// ── _startRound ───────────────────────────────────────────────────────────
describe('AttestationRound', function () { beforeEach(hookAt3853); afterEach(hookAt3913); describe('_startRound()', function () { it('proceeds when the request fee meets the provider min_fee exactly', async function () {
            let myPubkey = 'aa'.repeat(32);
            let capSS = { getSnapshot: sinon.stub().resolves({ validators: [{ pubkey: myPubkey }] }) };
            let hub   = makeHub({ capabilitySnapshot: capSS });
            hub.getIdentity = () => makeIdentity(myPubkey);
...
// ── _startRound ───────────────────────────────────────────────────────────
describe('AttestationRound', function () { beforeEach(hookAt3853); afterEach(hookAt3913); describe('_startRound()', function () { it('proceeds for a feeless request when min_fee is 0 (default)', async function () {
            let myPubkey = 'aa'.repeat(32);
            let capSS = { getSnapshot: sinon.stub().resolves({ validators: [{ pubkey: myPubkey }] }) };
            let hub   = makeHub({ capabilitySnapshot: capSS });
            hub.getIdentity = () => makeIdentity(myPubkey);
...
// ── _startRound ───────────────────────────────────────────────────────────
describe('AttestationRound', function () { beforeEach(hookAt3853); afterEach(hookAt3913); describe('_startRound()', function () { it('records an error in rounds when provider fetch throws', async function () {
            let myPubkey = 'aa'.repeat(32);
            let capSS = { getSnapshot: sinon.stub().resolves({ validators: [{ pubkey: myPubkey }] }) };
            let hub   = makeHub({ capabilitySnapshot: capSS });
            hub.getIdentity = () => makeIdentity(myPubkey);
...
// ── _startRound ───────────────────────────────────────────────────────────
describe('AttestationRound', function () { beforeEach(hookAt3853); afterEach(hookAt3913); describe('_startRound()', function () { it('records round state and calls consensus.propose when responsible', async function () {
            let myPubkey = 'aa'.repeat(32);
            let capSS = { getSnapshot: sinon.stub().resolves({ validators: [{ pubkey: myPubkey }] }) };
            let hub   = makeHub({ capabilitySnapshot: capSS });
            hub.getIdentity = () => makeIdentity(myPubkey);
...
// ── _startRound ───────────────────────────────────────────────────────────
describe('AttestationRound', function () { beforeEach(hookAt3853); afterEach(hookAt3913); describe('_startRound()', function () { it('snapshots the block-anchored model: pinnedModel into fetch, pinnedJudgeModel into roundState', async function () {
            let myPubkey = 'aa'.repeat(32);
            let capSS = { getSnapshot: sinon.stub().resolves({ validators: [{ pubkey: myPubkey }] }) };
            let hub   = makeHub({ capabilitySnapshot: capSS });
            hub.getIdentity = () => makeIdentity(myPubkey);
...
// ── _startRound ───────────────────────────────────────────────────────────
describe('AttestationRound', function () { beforeEach(hookAt3853); afterEach(hookAt3913); describe('_startRound()', function () { it('hands the provider this hub validated network, for the http_get SSRF hatch gate', async function () {
...
// ── _startRound ───────────────────────────────────────────────────────────
describe('AttestationRound', function () { beforeEach(hookAt3853); afterEach(hookAt3913); describe('_startRound()', function () { it('gives an unconfigured hub a 20 s provider fetch budget, not 10 s (operator ruling 2026-09-11)', async function () {
