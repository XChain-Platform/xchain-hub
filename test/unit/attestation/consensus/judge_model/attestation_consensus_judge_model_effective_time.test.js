...
// A mirror-era judge_model round can fail to finalize with the shape reproduced from a public testnet
// federation under this exact configuration: four validators hold an identical status=ok body against a redundancy of 3, the elected
// leader reaches agree() and broadcasts a PREPARE, and every follower logs:
//
// PREPARE effective_time <n> out of window from <leader> for <rid> (rejected)
//
// on a two-minute cadence indefinitely, while byte_equality rounds finalize on the same hubs.
// THE MECHANISM IS THE JUDGE CALL'S LATENCY, NOT THE WINDOW'S WIDTH. The leader picks its effective_time
// once, at proposal time, as now + forward. Under judge_model it then spends the whole judge round trip
// inside agree() before it establishes a winner and broadcasts. By the time a follower window-checks
// that stamp, the stamp has aged by the judge latency while the follower's own expectation moved forward
// with its clock. Once the gap exceeds ATTEST_RESPONSE_EFFECTIVE_TIME_SLACK_BEHIND_S, every follower
// refuses the PREPARE at once. The modeled round puts 108 s inside that gap against a 60 s slack.
// The two suites below pin both halves of the repair: the leader settles the stamp at winner ESTABLISHMENT
// (part 1), and the judge call is bounded so the ageing it can introduce is bounded too (part 2).
...
const JUDGE_S      = 108;   // the modeled judge latency
function mkIdentity() {
...
const T0    = 1788995902;                   // the modeled round's PROPOSE second
...
function makeRound(node) {
    return {
        ...
        // transport and never propose, which is the modeled set's shape.
...
function registerJudgeErrorStampTest() {
...
function registerJudgeEffectiveTimeCoreTests() {
...
describe('AttestationConsensus: a judge_model leader stamps effective_time at winner establishment', function () {
    ...
    registerJudgeEffectiveTimeCoreTests();

    registerJudgeEffectiveTimeEdgeTests();
});

function registerJudgeTimeoutEdgeTests() {
...
function registerJudgeTimeoutCoreTest() {
...
describe('llm.agree: the judge call is bounded by options.timeoutMs, measured from entry', function () {
    ...
    registerJudgeTimeoutCoreTest();

    registerJudgeTimeoutEdgeTests();
});
