...
// like it did.
//
// Many guards in test/unit are byte-identity or value-parity checks against a
// SIBLING checkout (the attestation and cross-chain-call twins against
// xchain-indexer, the chain registry against xchain-wallet, the oracle band
// constants against every mirror repo, and so on). Every one of them resolves
// its sibling with existsSync and calls this.skip() when it is absent. That is
// the right behaviour for a single-repo checkout, and it is invisible: mocha
...
// this repo's then-declared siblings ran 3723 passing / 5 pending where the
// full checkout runs 3724 / 4, so 1 guard had silently not run.
//
...
const SIBLINGS = [
    ...
    { repo: 'xchain-sdk', envs: [],
      marker: path.join('src', 'protocol', 'constants.js'),
      guards: 'the canonical equivocation-checkpoint twin and the oracle band constant mirror' },
    { repo: 'xchain-wallet', envs: [],
      marker: path.join('packages', 'core', 'src', 'registry', 'descriptors', 'index.js'),
      guards: 'the chain-registry snapshot guard' },
    { repo: 'xchain-vm', envs: [],
      marker: path.join('src', 'protocol', 'constants.js'),
