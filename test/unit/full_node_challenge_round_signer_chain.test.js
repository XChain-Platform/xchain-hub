...
function makeSpies() {
  return {
    ...
    wallet: {
      signPsbt: sinon.stub().returns({
        txHex: 'hex-1',
        txid: 'f'.repeat(64)
      }),
      signRevealPsbt: sinon.stub().returns({
        txHex: 'hex-2',
        txid: 'e'.repeat(64)
      })
    }
...
function registerFullNodeChallengeRoundSignerChainGateSuite1Part1() {
  ...
  it('refuses to wire the DOGE template, and a verdict then spends nothing', async function () {
    ...
    assert.strictEqual(eng.getBalanceFn, undefined, 'a DOGE balance must not be read as the BTC publisher wallet');
    await assert.rejects(() => eng.broadcastVerdict('NODEPROOF|wire'), /no broadcast pipeline/);
...
function registerFullNodeChallengeRoundSignerChainGateSuite1Part2() {
  it('refuses at send time when a DOGE-tagged hook was wired directly', async function () {
    ...
    let warned = await captureWarn(async () => {
      await assert.rejects(() => eng.broadcastVerdict('NODEPROOF|wire'), /REFUSING to publish a NODEPROOF verdict/);
    });
...
function registerFullNodeChallengeRoundSignerChainGateSuite1Part3() {
  it('does not reserve spend budget or claim the round on a wrong-chain wiring', async function () {
    ...
    assert.strictEqual(eng.rounds.get(epoch).finalized, false, 'the round stays unclaimed so a correctly-configured restart can still publish it');
    assert.strictEqual(spies.wallet.signPsbt.callCount, 0);
    assert.strictEqual(encoder.getUtxos.callCount, 0);
  });
}
describe('FullNodeChallengeRound signer chain gate', function () {
  registerFullNodeChallengeRoundSignerChainGateSuite1Part1.call(this);
  registerFullNodeChallengeRoundSignerChainGateSuite1Part2.call(this);
  registerFullNodeChallengeRoundSignerChainGateSuite1Part3.call(this);
});
...
describe('FullNodeChallengeRound runEpoch opens its round after the pinned microtask turns', function () {
  ...
  function timeRunEpoch(eng) {
    let flag;
    return turnsUntil(() => {
      flag = settleFlag(eng.runEpoch(288, 300));
    }, {
