...
function aCanonicalCapabilityMissingFromTheConfigEnSuite1Without(cap) {
  let caps = canonicalCaps();
  delete caps[cap];
  return caps;
}
function registerACanonicalCapabilityMissingFromTheConfigEnSuite1Part1() {
...
function registerACanonicalCapabilityMissingFromTheConfigEnSuite1Part2() {
...
function loadCapabilityConfigFileIntegrationSuite2WriteConfig(caps) {
  loadCapabilityConfigFileIntegrationSuite2TmpPath = path.join(os.tmpdir(), 'xc092_caps_' + process.pid + '_' + Math.random().toString(36).slice(2) + '.json');
  fs.writeFileSync(loadCapabilityConfigFileIntegrationSuite2TmpPath, JSON.stringify({
    CAPABILITIES: caps
  }));
  return loadCapabilityConfigFileIntegrationSuite2TmpPath;
}
function registerLoadCapabilityConfigFileIntegrationSuite2Part1() {
  afterEach(function () {
    if (loadCapabilityConfigFileIntegrationSuite2TmpPath) {
      try {
        fs.unlinkSync(loadCapabilityConfigFileIntegrationSuite2TmpPath);
      } catch (_) {}
    }
  });
  it('refuses a divergent config file on mainnet WITHOUT merging it', function () {
    ...
    let caps = canonicalCaps();
    caps.cross_chain.MIN_STAKE = '1000.00000000';
    expect(() => hub.loadCapabilityConfigFile(loadCapabilityConfigFileIntegrationSuite2WriteConfig(caps))).to.throw().with.property('code', 'MIN_STAKE_MISMATCH');
  ...
  it('refuses a config file with a missing capability WITHOUT merging it', function () {
    ...
    let caps = canonicalCaps();
    delete caps.full_node;
    expect(() => hub.loadCapabilityConfigFile(loadCapabilityConfigFileIntegrationSuite2WriteConfig(caps))).to.throw().with.property('code', 'CAPABILITY_UNCONFIGURED');
    expect(hub.p2pConfig && hub.p2pConfig.CAPABILITIES || undefined).to.equal(undefined);
  });
  ...
  it('halts startCapabilities instead of booting into a hub that fails every round', async function () {
    ...
    let caps = canonicalCaps();
    delete caps.full_node;
    let p = loadCapabilityConfigFileIntegrationSuite2WriteConfig(caps);
    let err = null;
    try {
      await hub.startCapabilities(p);
    } catch (e) {
      err = e;
    }
    expect(err).to.not.equal(null);
  ...
  it('loads a canonical config file on mainnet', function () {
    ...
    try {
      let hub = makeHub('mainnet');
      hub.loadCapabilityConfigFile(loadCapabilityConfigFileIntegrationSuite2WriteConfig(canonicalCaps()));
      expect(hub.p2pConfig.CAPABILITIES.cross_chain.MIN_STAKE).to.equal(CANONICAL.cross_chain.MIN_STAKE);
    } finally {
      logStub.restore();
    }
  });
}
let xChainHubAssertCanonicalMinStakesSuite3WarnStub;
function registerXChainHubAssertCanonicalMinStakesSuite3Part1() {
  beforeEach(function () {
    xChainHubAssertCanonicalMinStakesSuite3WarnStub = sinon.stub(console, 'warn');
  });
  afterEach(function () {
    xChainHubAssertCanonicalMinStakesSuite3WarnStub.restore();
...
function registerXChainHubAssertCanonicalMinStakesSuite3Part2() {
  ...
  describe('a canonical capability missing from the config entirely', function () {
    registerACanonicalCapabilityMissingFromTheConfigEnSuite1Part1.call(this);
    registerACanonicalCapabilityMissingFromTheConfigEnSuite1Part2.call(this);
  });
