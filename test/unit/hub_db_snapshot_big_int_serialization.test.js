...
const ROUTES = [{table:'price_snapshots',row:{id:1,effective_time:12345678901234567890n}}, {table:'oracle_prices',row:{id:1,effective_at:12345678901234567890n}}, {table:'cross_chain_matches',row:{id:1,status:'ok',effective_time:12345678901234567890n}}, {table:'capability_snapshots',row:{id:1,snapshot_block:12345678901234567890n}}, {table:'cross_chain_calls',row:{id:1,status:'ok',effective_time:12345678901234567890n}}, {table:'state_checkpoints',row:{id:1,checkpoint_seq:12345678901234567890n}}, {table:'anchor_reward_attestations',row:{id:1,snapshot_block:12345678901234567890n}}, {table:'attestation_responses',row:{id:1,effective_time:12345678901234567890n}}];
...
function makeFakeDb() {
  return {...DB_METHODS,async doQuery(sql){let text=String(sql);let hit=ROUTES.find(r=>text.includes('FROM '+r.table));return hit?[{...hit.row}]:[];}};
}
  ...
  return new Promise((resolve, reject) => {
    let req = http.request({host:'127.0.0.1',port:port,path:path,method:'GET'}, res => {
      ...
      res.on('end', () => {
        ...
        } catch (e) {/* left null on a bad body */}
...
let hubDbSnapshotRoutesBIGINTSerializationMatcSuite1Port, hubDbSnapshotRoutesBIGINTSerializationMatcSuite1Server;
function registerHubDbSnapshotRoutesBIGINTSerializationMatcSuite1Part1() {
  this.timeout(15000);
}
function registerHubDbSnapshotRoutesBIGINTSerializationMatcSuite1Part2() {
  before(async function () {
    ...
    mockWsLib.Server = function () {
      return {on:sinon.stub(),close:sinon.stub()};
    };
    mockWsLib.OPEN = 1;
    let origEnv = {};
    let envVars = {HUB_DB_HOST:'localhost',HUB_DB_PORT:'3306',HUB_DB_NAME:'testdb',HUB_DB_USER:'root',HUB_DB_PASS:'pass',HUB_PORT:'0',HUB_HOST:'127.0.0.1',HUB_API_KEY:'',HUB_ALLOW_UNAUTHENTICATED:'true',TELEMETRY_ENABLED:'false'};
    ...
    hubDbSnapshotRoutesBIGINTSerializationMatcSuite1Server = await waitForServer(() => capturedServer && capturedServer.listening ? capturedServer : null);
    hubDbSnapshotRoutesBIGINTSerializationMatcSuite1Port = hubDbSnapshotRoutesBIGINTSerializationMatcSuite1Server.address().port;
  });
}
function registerHubDbSnapshotRoutesBIGINTSerializationMatcSuite1Part3() {
  after(function () {
    if (hubDbSnapshotRoutesBIGINTSerializationMatcSuite1Server) {
      try {
        hubDbSnapshotRoutesBIGINTSerializationMatcSuite1Server.close();
      } catch (e) {/* already closed */}
    }
    sinon.restore();
  });
}
function registerHubDbSnapshotRoutesBIGINTSerializationMatcSuite1Part4() {
  for (const {table,row} of ROUTES) {
    it('GET /hub-db/snapshot/' + table + ' serves a real BigInt without a 500', async function () {
      let res = await get(hubDbSnapshotRoutesBIGINTSerializationMatcSuite1Port, '/hub-db/snapshot/' + table);
...
describe('hub-db snapshot routes: BIGINT serialization matches the WS path', function () {
  registerHubDbSnapshotRoutesBIGINTSerializationMatcSuite1Part1.call(this);
  registerHubDbSnapshotRoutesBIGINTSerializationMatcSuite1Part2.call(this);
  registerHubDbSnapshotRoutesBIGINTSerializationMatcSuite1Part3.call(this);
  registerHubDbSnapshotRoutesBIGINTSerializationMatcSuite1Part4.call(this);
});
