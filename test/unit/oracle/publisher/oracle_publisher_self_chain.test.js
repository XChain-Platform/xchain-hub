...
function makePublisher(opts) {
  ...
  let hub = {
    p2pConfig: Object.assign({
      PUBLISHER_QUEUE_PATH: path.join(dir, 'publisher-queue.jsonl')
    }, opts.cfg || {}),
    network: 'regtest',
    db: null,
    getIdentity: () => ({
...
function registerSelectInputsSuite1Part1() {
...
function registerSelectInputsSuite1Part2() {
  it('takes the depth bound from config', function () {
    const {
      p
    } = makePublisher({
      cfg: {
        ORACLE_PUBLISH_SELF_CHAIN_MAX_DEPTH: '1'
      }
    });
  ...
  it('leaves the regtest escape hatch exactly as it was', function () {
    const {
      ...
      cfg: {
        ORACLE_PUBLISH_ALLOW_UNCONFIRMED_INPUTS: 'true'
      }
    });
...
function registerSelectInputsSuite1Part3() {
...
function throughAPublishPassSuite2DustAwareEncoder(state) {
  ...
  return {
    ...
    createTx: sinon.stub().callsFake(async ({
      ...
      state.builds.push({
        inputs: spendable.map(u => u.txid),
        unconfirmed: !!unconfirmed
      });
      if (total < FEE) {
        let e = new Error('insufficient funds: selected inputs total ' + total + ' but ' + FEE + ' is required');
    ...
    broadcastTx: sinon.stub().callsFake(async () => {
      ...
      state.utxos = state.utxos.filter(u => Number(u.confirmations) < 1 || u.value === DUST).concat([utxo(txid, 0, BIG)]);
      return {
        txid
      };
    })
...
function throughAPublishPassSuite2Seed(h, rounds) {
  fs.writeFileSync(h.queuePath, rounds.map(r => JSON.stringify({
    round: r,
    btcBlockTime: 1800000000 + r * 600,
    btcBlockHeight: 900000 + r,
    prices: [{
      coinPair: 'BTC/USD',
      price: '60000.12'
    }],
...
function registerThroughAPublishPassSuite2Part1() {
  it('publishes every wire of a sweep instead of stalling once the confirmed outputs run out', async function () {
    ...
    const state = {
      utxos: [utxo('f1'.repeat(32), 6, BIG), utxo('d1'.repeat(32), 6, DUST), utxo('d2'.repeat(32), 6, DUST)],
      builds: []
    };
    ...
    expect(fs.existsSync(queuePath) && fs.readFileSync(queuePath, 'utf8').trim(), 'the queue drains').to.equal('');
  });
}
function registerThroughAPublishPassSuite2Part2() {
...
function registerOraclePublisherSpendingItsOwnChangeInsideOSuite3Part1() {
  afterEach(function () {
    sinon.restore();
    for (let d of tmpDirs) {
      try {
        fs.rmSync(d, {
          recursive: true,
          force: true
  ...
  describe('selectInputs', function () {
    registerSelectInputsSuite1Part1.call(this);
    registerSelectInputsSuite1Part2.call(this);
    registerSelectInputsSuite1Part3.call(this);
  });
  describe('through a publish pass', function () {
    registerThroughAPublishPassSuite2Part1.call(this);
    registerThroughAPublishPassSuite2Part2.call(this);
  });
}
describe('OraclePublisher: spending its own change inside one pass', function () {
  registerOraclePublisherSpendingItsOwnChangeInsideOSuite3Part1.call(this);
});
