...

{
    ...
    function rejectsCanonicalBase64ThatIsNotTest16(){
    ...
    function rejectsATruncatedDeflateStreamTest17(){
    ...
    function acceptsMultiByteUtf8ThatTest20(){
    ...
    function neverFallsBackToTreatingTheTest21(){
    ...
    function everyFailureReasonIsDistinctAndTest22(){
    ...
    function priceBatchCompressionEveryFailureIsSuite15() {
        ...
        it('rejects canonical base64 that is not a deflate stream', rejectsCanonicalBase64ThatIsNotTest16);
        it('rejects a truncated deflate stream', rejectsATruncatedDeflateStreamTest17);
        it('rejects a stream that inflates to zero bytes', rejectsAStreamThatInflatesToTest18);
        it('rejects inflated bytes that are not valid UTF-8', rejectsInflatedBytesThatAreNotTest19);
        it('accepts multi-byte UTF-8 that round trips exactly', acceptsMultiByteUtf8ThatTest20);
        it('never falls back to treating the field as an uncompressed body', neverFallsBackToTreatingTheTest21);
        it('every failure reason is distinct and stable', everyFailureReasonIsDistinctAndTest22);
    }

    describe('price_batch_compression: every failure is explicit and terminal @regression', priceBatchCompressionEveryFailureIsSuite15);
