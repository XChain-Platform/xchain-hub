...
let oracleConsensusSnapshotMembershipGateOraclSuite1Hub, oracleConsensusSnapshotMembershipGateOraclSuite1Pm, oracleConsensusSnapshotMembershipGateOraclSuite1Oc, oracleConsensusSnapshotMembershipGateOraclSuite1OracleRound;
// the leader now derives from the locked snapshot (validators 1+2),
// so round 2 -> sorted-pubkeys[2 % 2] = validator-1 (this hub).
const oracleConsensusSnapshotMembershipGateOraclSuite1ROUND = 2;
function registerOracleConsensusSnapshotMembershipGateOraclSuite1Part1() {
  beforeEach(function () {
    oracleConsensusSnapshotMembershipGateOraclSuite1Hub = createMockHub();
    oracleConsensusSnapshotMembershipGateOraclSuite1Pm = oracleConsensusSnapshotMembershipGateOraclSuite1Hub._peerManager;
    oracleConsensusSnapshotMembershipGateOraclSuite1Pm.validatorAddr = VALIDATORS_3[0].addr;
    oracleConsensusSnapshotMembershipGateOraclSuite1Pm.validatorPubkeys = registryFor([...VALIDATORS_3, NONMEMBER]);
    // Own identity = validator-1's registered pubkey so self resolves consistently.
    oracleConsensusSnapshotMembershipGateOraclSuite1Hub._identity.getPubkeyHex.returns(VALIDATORS_3[0].pubkey);
    // Count-mode snapshot: only validators 1+2 qualify for `price`.
    oracleConsensusSnapshotMembershipGateOraclSuite1Hub.capabilitySnapshot = {
    ...
    oracleConsensusSnapshotMembershipGateOraclSuite1OracleRound = {
      getSubmissions: sinon.stub().returns(new Map()),
      priceFetcher: null
    };
    oracleConsensusSnapshotMembershipGateOraclSuite1Oc = new OracleConsensus(oracleConsensusSnapshotMembershipGateOraclSuite1Hub, oracleConsensusSnapshotMembershipGateOraclSuite1OracleRound);
    oracleConsensusSnapshotMembershipGateOraclSuite1Oc.setValidatorSet(VALIDATORS_3);
  });
  afterEach(function () {
    sinon.restore();
  });
  it('finalizeRound excludes a non-member submission from the proposed aggregate', async function () {
    oracleConsensusSnapshotMembershipGateOraclSuite1OracleRound.getSubmissions.returns(buildSubmissions([{
    ...
    {
      sender: NONMEMBER.addr,
      prices: [{
        coinPair: 'BTC/USD',
        price: '999999'
      }]
    }]));
    await oracleConsensusSnapshotMembershipGateOraclSuite1Oc.finalizeRound(oracleConsensusSnapshotMembershipGateOraclSuite1ROUND, 100, 1700000000);
    expect(oracleConsensusSnapshotMembershipGateOraclSuite1Pm.broadcast.calledOnce).to.be.true;
    let [type, data] = oracleConsensusSnapshotMembershipGateOraclSuite1Pm.broadcast.firstCall.args;
...
function registerOracleConsensusSnapshotMembershipGateOraclSuite1Part2() {
  it('finalizeRound skips the round when member submissions fall below minSubmissions', async function () {
    let skipped = sinon.stub(oracleConsensusSnapshotMembershipGateOraclSuite1Oc, 'storeSkippedRound').resolves();
    oracleConsensusSnapshotMembershipGateOraclSuite1OracleRound.getSubmissions.returns(buildSubmissions([{
      sender: VALIDATORS_3[0].addr,
      prices: [{
        coinPair: 'BTC/USD',
    ...
    // Two non-members padding the raw count past minSubmissions=2.
    {
      ...
      sender: makeValidator(8).addr,
      prices: [{
        coinPair: 'BTC/USD',
        price: '100002'
      }]
    }]));
    oracleConsensusSnapshotMembershipGateOraclSuite1Pm.validatorPubkeys.set(makeValidator(8).addr, makeValidator(8).pubkey);
    await oracleConsensusSnapshotMembershipGateOraclSuite1Oc.finalizeRound(oracleConsensusSnapshotMembershipGateOraclSuite1ROUND, 100, 1700000000);
    expect(oracleConsensusSnapshotMembershipGateOraclSuite1Pm.broadcast.called).to.be.false;
  ...
  it('count-mode quorum ignores votes from senders outside the snapshot', function () {
    ...
    expect(oracleConsensusSnapshotMembershipGateOraclSuite1Oc.quorumMet(pending, votes)).to.be.false;
    // Second member joins: met.
    votes.add(VALIDATORS_3[1].pubkey);
    expect(oracleConsensusSnapshotMembershipGateOraclSuite1Oc.quorumMet(pending, votes)).to.be.true;
  });
}
function registerOracleConsensusSnapshotMembershipGateOraclSuite1Part3() {
  it('count-mode quorum counts one key once however many addrs carried it', function () {
    ...
    expect(oracleConsensusSnapshotMembershipGateOraclSuite1Oc.quorumMet(pending, votes)).to.be.false; // one distinct key, not two
  });
  it('legacy behavior preserved when no snapshot is available (memberPubkeys null)', function () {
    ...
    expect(oracleConsensusSnapshotMembershipGateOraclSuite1Oc.quorumMet(pending, votes)).to.be.true; // raw sender count, unchanged
  });
  ...
  it('records validator_count as the deduped member tally, not the raw prepare set', async function () {
    ...
    await oracleConsensusSnapshotMembershipGateOraclSuite1Oc.finalizeCommittedRound(oracleConsensusSnapshotMembershipGateOraclSuite1ROUND);
...
function registerOracleConsensusSnapshotMembershipGateOraclSuite1Part4() {
  it('_handlePropose locks memberPubkeys on the pending round', async function () {
    ...
    let pending = oracleConsensusSnapshotMembershipGateOraclSuite1Oc.pendingRounds.get(oracleConsensusSnapshotMembershipGateOraclSuite1ROUND);
    expect(pending).to.exist;
    expect(pending.memberPubkeys).to.be.instanceOf(Set);
    expect([...pending.memberPubkeys]).to.have.members([VALIDATORS_3[0].pubkey, VALIDATORS_3[1].pubkey]);
    // A non-member PREPARE must not advance the round to COMMIT quorum.
    oracleConsensusSnapshotMembershipGateOraclSuite1Oc.handlePrepare({
      sender: NONMEMBER.addr,
      sig_pubkey: NONMEMBER.pubkey,
      data: {
        round: oracleConsensusSnapshotMembershipGateOraclSuite1ROUND,
...
describe('OracleConsensus: snapshot membership gate (Oracle M1)', function () {
  registerOracleConsensusSnapshotMembershipGateOraclSuite1Part1.call(this);
  registerOracleConsensusSnapshotMembershipGateOraclSuite1Part2.call(this);
  registerOracleConsensusSnapshotMembershipGateOraclSuite1Part3.call(this);
  registerOracleConsensusSnapshotMembershipGateOraclSuite1Part4.call(this);
});
