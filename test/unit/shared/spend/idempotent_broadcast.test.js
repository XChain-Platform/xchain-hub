...
function registerIsAmbiguousSendErrorSuite1Part1() {
  ...
  // exactly the case where the tx MAY ALREADY have reached the node, and it was once
  // classified retry-safe alongside 'bad-txns-inputs-missingorspent'.
...
function registerIsAmbiguousSendErrorSuite1Part2() {
  it('keeps a sub-500 HTTP refusal DEFINITIVE even when it carries that message', function () {
    ...
    expect(isAmbiguousSendError(Object.assign(new Error('Encoder RPC error: Transaction broadcast failed'), {
      response: {
        status: 400
      }
    }))).to.equal(false);
  ...
  it('leaves the prefix-less classification untouched', function () {
    ...
    expect(isAmbiguousSendError(Object.assign(new Error('Transaction broadcast failed'), {
      code: 'ECONNREFUSED'
    }))).to.equal(false);
  });
  ...
  it('treats a post-funding failure as AMBIGUOUS whatever shape the rejection has', function () {
    expect(isAmbiguousSendError(Object.assign(new Error('Encoder RPC error: bad-txns-inputs-missingorspent'), {
      fundsCommitted: true,
      phase1Txid: 'a'.repeat(64)
    }))).to.equal(true);
    expect(isAmbiguousSendError(Object.assign(new Error('Encoder RPC error: no UTXOs available'), {
      fundsCommitted: true,
      response: {
        status: 400
      }
    }))).to.equal(true);
    expect(isAmbiguousSendError(Object.assign(new Error('connect ECONNREFUSED'), {
      fundsCommitted: true,
      code: 'ECONNREFUSED'
    }))).to.equal(true);
  });
...
function registerBroadcastOnceSuite2Part1() {
  afterEach(function () {
    SpendGuard.unregister('BO');
  });
...
function registerBroadcastOnceSuite2Part2() {
  ...
  it('tags an ambiguous send error, does NOT mark the key, and CHARGES the window', async function () {
    ...
    try {
      await broadcastOnce({
        ...
        guard,
        cost: 250,
        ambiguousTag: 'anchorAmbiguousSend',
        send: async () => {
...
function registerBroadcastOnceSuite2Part4() {
  it('hands the reserved budget back when the send fails definitively', async function () {
    ...
    try {
      await broadcastOnce({
        ...
        cost: 100,
        send: async () => {
          throw new Error('Encoder RPC error: bad-txns-inputs-missingorspent');
        }
      });
    } catch (e) {/* expected */}
  ...
  it('does NOT tag a definitive send error', async function () {
    ...
    try {
      await broadcastOnce({
        ...
        ambiguousTag: 'anchorAmbiguousSend',
        send: async () => {
          throw new Error('Encoder RPC error: bad tx');
        }
      });
    } catch (e) {
      err = e;
...
function registerIdempotentBroadcastHelperSuite3Part1() {
  describe('isAmbiguousSendError', function () {
    registerIsAmbiguousSendErrorSuite1Part1.call(this);
    registerIsAmbiguousSendErrorSuite1Part2.call(this);
  });
  ...
  describe('broadcastOnce', function () {
    registerBroadcastOnceSuite2Part1.call(this);
    registerBroadcastOnceSuite2Part2.call(this);
    registerBroadcastOnceSuite2Part3.call(this);
    registerBroadcastOnceSuite2Part4.call(this);
  });
}
describe('idempotent-broadcast helper', function () {
  registerIdempotentBroadcastHelperSuite3Part1.call(this);
});
