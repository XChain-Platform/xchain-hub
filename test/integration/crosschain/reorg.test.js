...
describe('Integration: Reorg Handling (SC-5.x)', function () {
    ...
    describe('SC-5.1: Rollback cascading writes', registerRollbackTests);
    describe('SC-5.2: Reorg with multi-validator consensus', registerConsensusReorgTests);
    describe('SC-5.3: Duplicate reorg prevention', registerDuplicateReorgTests);
});

// SC-5.1: Reorg rollback of attestations and price disputes
function registerRollbackTests() {
        it('deletes attestations after reorg timestamp and disputes price snapshots', async function () {
            ...
            await seedReorgFixtures(db, reorgTimestamp);
            ...
            await assertReorgRollback(db, reorgEvent);

            await reorgHandler.stop();
        });
}

async function seedReorgFixtures(db, reorgTimestamp) {
...
async function assertReorgRollback(db, reorgEvent) {
    ...
    for (let att of remainingAtts) expect(att.source_action_index).to.be.at.most(2);
...
function registerConsensusReorgTests() {
        it('finalizes reorg after PREPARE and COMMIT quorum', async function () {
            ...
            let reorgHandler = await createReorgHandler(hub, VALIDATORS_4);
...
async function createReorgHandler(hub, validators) {
    let reorgHandler = new ReorgHandler(hub);
    sinon.stub(reorgHandler, 'verifyReorgAgainstOwnNode').resolves(true);
    reorgHandler.setValidatorSet(validators);
    await reorgHandler.start();
    return reorgHandler;
}

function emitReorgPrepares(hub, reorgId, reorgTimestamp, digest) {
    for (let i = 1; i < 3; i++) {
...
function emitReorgCommits(hub, reorgId, digest) {
    for (let i = 1; i < 3; i++) {
        hub._peerManager.emit('message', buildEnvelope('XCHAIN_REORG_COMMIT', {
            reorgId: reorgId, digest: digest
        }, VALIDATORS_4[i].addr));
    }
}

// SC-5.3: Duplicate reorg report prevention
function registerDuplicateReorgTests() {
