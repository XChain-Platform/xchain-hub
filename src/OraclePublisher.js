...
class OraclePublisher {
    ...
    constructor(hub) {
        ...
        // Durable at-most-once. The in-process Set above vanishes on restart, but the
        // finalized round can still be on the durable JSONL queue, so a restart before
        // the queue rewrite is repaired would re-broadcast an already-paid round
        ...
        // durable guard is inert and the in-process Set is the only at-most-once cover.
    ...
    async _hydratePublishedMarkers() {
        ...
        for (let r of (rows || [])) {
            ...
            if (r.sent_at !== null && r.sent_at !== undefined) {
                this._publishedRounds.add(round);
            } else {
    ...
    async _processQueue() {
        ...
        for (let entry of entries) {
            ...
            try {
                ...
                // in-process Set above does not). Best-effort: on failure the intent row
                // remains and a restart quarantines the round rather than re-broadcasting.
