...
### Security
- `src/PriceAggregator.js` — `receiveValidatedRound()` now re-verifies a pushed PRICE v0 round at the hub instead of trusting the pusher's claim that PBFT signatures were already checked locally. Previously the caller-supplied `sigs` array was stringified verbatim into `consensus_proof`, its raw array length became `validator_count`, and every pair was inserted into `price_snapshots` with `status='finalized'` — so any caller holding the shared write API key could inject an arbitrary fabricated price that downstream consumers (WS-broadcast indexer mirrors, native-coin fee validation, FIAT dispensers, the public price RPCs and snapshot endpoints) treated as authoritative quorum output, indistinguishable from a real round. The handler now (1) validates the wire shape strictly (round / timestamp / block_index presence, on-chain pair-format rules, `{pubkey, sig}` hex signature entries), (2) resolves the deterministic price-capability validator snapshot at the round's `block_index` via `CapabilitySnapshot`, failing closed with `validator snapshot unavailable` when the indexer cannot be reached, (3) verifies each Ed25519 signature over the canonical round payload (byte-identical to `xchain-indexer/src/ed25519.js` `buildPriceV0Payload` — see the matching builders in `OracleConsensus`), counting at most one signature per qualified pubkey and skipping unknown/invalid sigs with the same semantics as the indexer's on-chain PRICE v0 parser so hub acceptance matches indexer acceptance, and (4) requires the verified count to meet the PBFT quorum `2*floor((N-1)/3)+1` over the snapshot size before any INSERT. `consensus_proof` now stores only the verified `(pubkey, sig)` pairs and `validator_count` is the verified-signature count, not the claimed one. `receiveOraclePrice()` (PRICE v1) carries no signatures on the wire by design — its authenticity is the on-chain transaction validated by the observing indexer — so it gains strict value/fee wire-format validation and an explicit trust-model comment instead of cryptographic verification. Regression coverage in `test/unit/PriceAggregator.test.js`: forged signatures, valid signatures replayed over substituted price data, signers outside the qualified set, sub-quorum rounds, duplicated-pubkey padding, snapshot-unavailable fail-closed (both null snapshot and absent snapshot machinery), single-node quorum-1 acceptance, and the previously-stored opaque-string sig shape are all rejected without touching the DB; a legitimately quorum-signed round is accepted and stores the verified proof. The PRICE v1 `effective_at` tests were also realigned to the uniform 24h delay shipped in the previous release.

### Added
- `test/helpers/mockExternalApi.js`, `test/integration/oracle/priceFetch.integration.test.js`, `test/fuzz/generators.js` — multi-fiat price-fetch coverage. `PriceFetcher` supports 12 fiats × 3 coins (36 pairs), but every mock fixture and the fuzz price generator previously exercised only the `usd` key (3 pairs), so 33 of 36 pairs — including high-magnitude currencies (KRW/JPY ~ millions per BTC) where `parseFloat` + `toFixed(8)` precision is most fragile — had no integration-level coverage and a precision regression in that range would have passed the whole suite undetected. Adds `FULL_COINGECKO_RESPONSE` / `FULL_CMC_RESPONSE` 12-fiat fixtures (values kept just below the `PRICE_MAX` validity cap), a new integration scenario (SC-1.7) asserting all 36 pairs are returned from a full dual-source response and spot-checking exact KRW/JPY values, and raises the fuzz `fc_price()` upper bound from `1_000_000` to `9_999_999` (just under `PRICE_MAX`) so property tests reach the high-magnitude range while staying inside the production validity filter. Test-only; no production code changed.
...
### Changed
- `src/XChainHub.js` — the BTC indexer-URL resolver now reads the indexer host from the renamed flat-config key `INDEXER_URL` (was `INDEXER_HOST`), tracking the platform-wide `<SERVICE>_URL` config/env naming convention. The preferred nested `{host, port, ...}` lookup is unchanged; only the legacy flat-config fallback key name changed. No behavioral change.
...
- `src/AttestationRound.js` — removed an empty `peerManager` `'message'` listener and the no-op `_handleMessage()` stub it dispatched to. `AttestationRound` is event-driven only through its indexer poll loop; all PBFT attestation traffic (PROPOSE/PREPARE/COMMIT) is handled by `AttestationConsensus`, which registers its own independent `'message'` listener. The `AttestationRound` listener performed no work — it ran an empty function for every inbound P2P message — and existed only as a forward-compatibility hook for subclasses that were never written. Removing it eliminates the per-message empty dispatch and the misleading impression that `AttestationRound` participates in message handling. `AttestationConsensus` is untouched and continues to handle all attestation PBFT messages; the `start()`/`stop()` poll-timer lifecycle is otherwise unchanged. No wire-format, schema, or behavioral change.

### Fixed
- `src/AttestationPublisher.js` — `ATTEST_WIRE_MAX_BYTES` was `8192`, mirroring the now-corrected encoder pre-check `MAX_DATA_BYTES`. A raw attestation payload of N bytes (N ≥ 256) compiles to N + 3 once the OP_PUSHDATA2 prefix is added, so 8190–8192-byte payloads passed this guard but were then rejected downstream by the encoder's compiled-size ceiling (8192). Lowered to `8189` — the largest raw payload that still fits the compiled ceiling — so an over-limit attestation is rejected here with a clear message instead of failing during transaction build. Keeps the constant in lock-step with `xchain-encoder/src/validator.js`.
- `src/XChainHub.js` — `FEE_PAYMENT_MODE` is no longer published to indexers as an operational param. The field was a member of `OPERATIONAL_PARAMS`, so the hub accepted, stored, and served a per-chain `FEE_PAYMENT_MODE` override pushed through governance — but the consuming indexer's runtime fee-mode detector derives the mode purely from the coin and the transaction's fee output and never reads the overlaid value, so the override had zero observable effect on any chain. Publishing a tunable that the consumer ignores is a silent-misconfiguration hazard: an operator following normal governance procedure got no signal that the change did nothing. Removing `FEE_PAYMENT_MODE` from `OPERATIONAL_PARAMS` (alongside the companion removal from `xchain-indexer`'s config overlay) makes the published governance surface match the behaviour the indexer actually implements. No other param is affected and no consensus path changes.
- `src/PriceFetcher.js` — `fetchFromCoinMarketCap()` now retries transient rate-limit responses with the same backoff as `fetchFromCoinGecko()`, restoring genuine dual-source price redundancy. The CoinGecko fetcher already retried up to 3 times with exponential backoff + jitter on HTTP 429/503, but the CoinMarketCap fetcher had a bare `try`/`catch` that returned `null` on the first error — so when an operator configured both API keys expecting two independent sources, a single transient CMC 429/503 silently dropped that source for the entire ~10-minute oracle round, leaving the round on CoinGecko alone with no warning. The shared retry loop is extracted into a new `_fetchWithRetry(url, options)` helper (3 attempts, retry only on 429/503, exponential backoff with jitter) that both fetchers now delegate to, so the two price sources have identical rate-limit resilience and the retry behaviour lives in one place. Behaviour is unchanged on success and on non-retryable errors; only the previously-missing CMC retry path is new. Covered by the existing `test/unit/PriceFetcher.test.js` single-source-failure cases (now asserting the post-retry warning) and `test/chaos/oracle/single-source-failure.chaos.test.js`.
