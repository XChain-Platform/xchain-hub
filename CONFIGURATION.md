...
| `HUB_RATE_LIMIT_RPM` | No | `100` | The PUBLIC budget: requests per minute per client IP for callers without a hub key, on every route EXCEPT `/hub-db/snapshot/*` (see below), on every role including a validator hub. A JSON-RPC batch costs one token per call, not one per HTTP request. Over the limit the hub answers `429` with a JSON-RPC error body (code `-32029`) naming the limit, the window and the wait, alongside `Retry-After` and `RateLimit-*` headers. |
| `HUB_AUTH_RATE_LIMIT_RPM` | No | `60000` | The AUTHENTICATED budget: requests per minute per client IP for callers whose `x-api-key` matches `HUB_API_KEY`, `HUB_REORG_API_KEY` or `HUB_CONFIG_SECRETS_API_KEY`, metered in its own bucket so the fleet's indexer push and replay traffic keeps the fleet value without lifting the throttle for keyless callers. A wrong key is metered as public. Batches are charged per call here too. Never defaults below `HUB_RATE_LIMIT_RPM`; an unparseable or non-positive value keeps the default. A keyless hub has no authenticated tier. |
| `HUB_SNAPSHOT_RATE_LIMIT_RPM` | No | `600` | Requests per minute per client IP on the mirror-bootstrap routes `/hub-db/snapshot/*`, metered in their OWN bucket so a bootstrapping indexer and ordinary polling cannot starve each other. Sized to the measured drain: about 30 page reads for one mirror's full ten-table bootstrap, about 96 for three mirrors behind one address, retried every 30s while any mirror is still draining. Over the limit the hub answers `429` in the `{ "error": ... }` shape these REST routes already use. An unparseable or non-positive value keeps the default. |
...
| `HUB_MAX_RPC_BATCH` | No | `20` | Maximum JSON-RPC calls in one batch array. The router dispatches every element concurrently, so an uncapped batch amplifies one request into hundreds of DB-touching handlers; the rate limits above charge each element, and this caps the count outright. Over the cap the hub answers `400` with JSON-RPC error `-32600`. An unparseable or non-positive value keeps the default. |
...
| `ORACLE_BATCH_LANDING_RESERVE_MS` | No | see `src/oracle/price_batch_cadence.js` | Time budgeted for a batch to broadcast, confirm and index, counted against the staleness bound when the window ceiling is derived. |
...
| `ATTESTATION_POLL_MS` | No | `3000` | How often the attestation round polls the indexer for new pending requests. The floor on how long a request waits before any validator starts work on it, and above `ATTEST_ZERO_CONF_ACTIVATION` (where the hub no longer waits for confirmations) it is the dominant term in that wait. Each poll is two indexer queries, the tip read and the pending page. |
...
| `ATTESTATION_FETCH_TIMEOUT` | No | `20000` | Per-request budget (ms) for one external provider fetch, and the same budget the `judge_model` strategy gives its judge call, so the two legs of a round agree on how long a slow vendor may be. Raised from `10000` by operator ruling: a healthy-but-slow provider that crossed 10 s was aborted, leaving the round with fewer independent bodies than its redundancy asked for, which `byte_equality` scores as a `no_quorum` rather than as a slow vendor. Keep it well under `ATTESTATION_ROUND_TIMEOUT_MS`, which stays the terminal backstop. A zero, a negative or a non-numeric value warns and falls back to the default. |
...
| `ATTESTATION_PROPOSER_SEEN_MAX` | No | `10000` | Cap on the `proposerSeen` ring (request id to set of pubkeys observed proposing for it), tracked separately from a live round so leader rotation still knows who has already spoken for a request after that round's own state was torn down on timeout. Same ring-bounded FIFO eviction rule as `ATTESTATION_TORNDOWN_MAX`, so requestId flooding cannot grow it without bound. |
...
| `ATTEST_BATCH_MAX_REFUSAL_ATTEMPTS` | No | `3` | How many times a window whose HEAD wire was refused **before it could be sent** (an encoder HTTP 4xx such as insufficient funds or unconfirmed change, or an `ECONNREFUSED`/`ENOTFOUND`/`EAI_AGAIN` that never connected) is rebuilt and re-proposed before it latches like any other broadcast failure. Nothing left the process on those shapes, so the publish-intent marker is withdrawn and the window is retried with byte-identical content instead of being quarantined for an operator; at the bound it latches once, keeps its intent marker and quarantines from then on. An ambiguous failure, and any failure on a wire after the head, never retries. Also resolves from `p2pConfig`; the env var wins. |
